...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        replace = self.input_string.replace('\n', ' ')
        replaced = replace.replace('\t', ' ')
        index = self.current_char_index
        search = re.search(' ', replaced[index:])
        temp = 0
        sliced = replaced[index:]
        if search == None:
            return 
        if search.start() == 0:
            while search is not None:  
                if sliced[temp] == ' ':
                    temp += 1
                    search = re.search(' ', sliced[temp:])
                else: 
                    break
        else:
            if sliced[temp + 1] == ' ' and temp != 0:
                temp += search.end()
            elif temp != 0:
                temp += 1
        self.current_char_index = index + temp
    ...
    def consume(self, *expected_tokens):
        ...
        if current_token[0] in expected_tokens:
            if current_token[0] in [Token.NUM, Token.ID]:
                return(current_token)
            else:
                return(current_token[0])
        else:
            self.unexpected_token(current_token, expected_tokens)        
...
class Boolean_Comparison_AST:
    def __init__(self, left, op, right, left2, op2, right2, booleanValue):
        self.left = left
        self.op = op
        self.right = right
        self.left2 = left2
    ...
    def indented(self, level):
        return indent(self.op, level) + \
               self.left.indented(level+1) + \
               self.right.indented(level+1) + \
               indent(self.boolean, level) + \
               indent(self.op2, level) + \
               self.left2.indented(level+1) + \
               self.right2.indented(level+1)            
               
class Boolean_Not_AST:
    def __init__(self, left, op, right, booleanValue):
        self.left = left
        self.op = op
        self.right = right
        self.boolean = booleanValue
    def __repr__(self):
        return repr(self.left) + self.op + repr(self.right)
    def indented(self, level):
        return indent(self.boolean, level) + \
               indent(self.op, level) + \
               self.left.indented(level+1) + \
               self.right.indented(level+1)
...
def if_statement():
    ...
    if scanner.lookahead() == 'ELSE':
        ...
        other = statements()
        end = scanner.consume(Token.END)
        return If_Else_AST(condition, then, other)
    else:
...
operator = { Token.LESS:'<', Token.EQ:'=', Token.GRTR:'>',
             ...
             Token.ADD:'+', Token.SUB:'-', Token.MUL:'*', Token.DIV:'/', Token.AND:'and', 
             Token.OR:'or', Token.NOT:'not' }

def comparison():
    if scanner.lookahead() in {Token.NOT}:
        booleanNot = scanner.consume(Token.NOT)
        left = expression()
        op = scanner.consume(Token.LESS, Token.EQ, Token.GRTR,
                             Token.LEQ, Token.NEQ, Token.GEQ)
        right = expression()
        return Boolean_Not_AST(left, operator[op], right, booleanNot)
    left = expression()
    op = scanner.consume(Token.LESS, Token.EQ, Token.GRTR,
                         Token.LEQ, Token.NEQ, Token.GEQ)
    right = expression()
    if scanner.lookahead() in {Token.AND, Token.OR, Token.NOT}:
        booleanValue = scanner.consume(Token.AND, Token.NOT, Token.OR)
        otherleft = expression() 
        op2 = scanner.consume(Token.LESS, Token.EQ, Token.GRTR,
                             Token.LEQ, Token.NEQ, Token.GEQ)
        otherright = expression()
        return Boolean_Comparison_AST(left, operator[op], right, otherleft, operator[op2], otherright, booleanValue)
